...
class ContactsTest extends TestCase
{
    ...
    /** @test * */
    public function fields_are_required_on_create()
    {
    ...
    /** @test * */
    public function fields_are_required_on_update()
    {
        $this->post('/api/contacts', $this->data());
        $contact = Contact::first();

        collect(['name', 'email', 'birthday', 'company'])
            ->each(function ($field) use ($contact) {

                $response = $this->patch('/api/contacts/' . $contact->id, array_merge($this->data(), [$field => '']));
    ...
    public function birthdays_are_properly_stored()
    {
        ...
        $this->assertEquals('1988-12-01', Contact::first()->birthday->format('Y-m-d'));
    }
